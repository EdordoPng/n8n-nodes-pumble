    ...
    if (returnAll) {
        ...
        do {
            ...
            const responseData = await pumbleApiRequest.call(
                this, 'GET', '/listUsers', {}, qs,
            );

            const users = (responseData.users ?? responseData) as IDataObject[];
            allResults.push(...(Array.isArray(users) ? users : [responseData]));
        ...
        const users = (responseData.users ?? responseData) as IDataObject[];
        const results = Array.isArray(users) ? users : [responseData];
